app.config(['$routeProvider', function($routeProvider) {
    ...
    .controller('InvcreateCtr', function($rootScope, $mdDialog, $filter, $q, $timeout, $http, $scope, $cacheFactory, ngTableParams, $mdToast, $mdDialog, $location, $routeParams) {
        ...
        if (pi) {
            $scope.Type = "Proformainvoice";
            $http({ url: 'proformadetails?ids=' + pi, method: 'GET', ignoreLoadingBar: true }).success(function(data) {
                $scope.form = data;
                $scope.form.Customer = data.customer;
        ...
        $scope.create = function() {
            ...
            // return;
            if ($scope.Type == 'Create' || $scope.Type == 'Proformainvoice') {
                if (rid) {
                    $scope.form.RID = rid;
                }
                newin();
        ...
        $scope.todelete = function(iid) {
            ...
            $mdDialog.show(confirm).then(function() {
                $http({ url: 'invoice/' + iid, method: 'DELETE' }).success(function(data) {
                    al('Deleted Successfully');
                    $location.path('/invoice');
                }).error(function(data, status) {
                    al('This Invoice in use. Cant Delate');
                });
            }, function() {
                al('Delete Cancelled');
            });
        ...
        $scope.edit = function(index) {
            $scope.curedit = angular.copy(index);
        ...
        var dialog = { scope: $scope, preserveScope: true, controller: function($scope, $mdDialog) { $scope.hide = function() { $mdDialog.hide(); if ($scope.Type == 'Edit') { $scope.custform = ""; }
                    $scope.formError = ''; }; }, clickOutsideToHide: true };
    ...
    .controller('PrintCtr', function($mdToast, $timeout, $filter, $window, $http, $scope, $cacheFactory, ngTableParams, $mdToast, $mdDialog, $location, $routeParams, $rootScope) {
        ...
        var dialog = { scope: $scope, preserveScope: true, controller: function($scope, $mdDialog) { $scope.hide = function() { $mdDialog.hide(); if ($scope.Type == 'Edit') { $scope.form = ""; }
                    $scope.formError = ''; }; }, clickOutsideToHide: true };
