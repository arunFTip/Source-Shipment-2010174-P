...
use App\CUSTOMER,Auth,App\INVOICE,App\PAYMENT,App\DETAILS,DB,App\SETTINGS,App\DC,App\PROFORMA,Mail;
...
class InvoiceController extends Controller
{
	public function index(Request $request)
	{
		...
		->select('IID','PRID','InvNo','Date','Due','InvThr','InvTy','Job' ,'invoice.CID','Total','Balance','Status','CName','Mobile1');
		...
		if(isset($filters['Search'])&&$filters['Search']!='')
		{
			$data->where('CName','LIKE','%'.$filters['Search'].'%')
				 ...
				 ->orWhere('InvNo','LIKE','%'.$filters['Search'].'%');
		}
        ...
        if($request->sort)
        {
         	$data->orderByRaw("DATE(from_unixtime(Date)) DESC")->orderBy('IID','DESC')->orderBy($request->sort[0]['field'],$request->sort[0]['dir']);	
        }
        $data=$data->get();
	...
	public function store(Request $request)
	{
		...
		if(!isset($input['RID']))
		{
			...
			$Job=INVOICE::whereBetween(DB::raw("DATE(from_unixtime(Date))"),[Carbon::parse($from), Carbon::parse($to)])->where('InvFrom',$input['InvFrom'])->where('InvTy',$input['InvTy'])->orderBy('Job','DESC')->first();
		...
		$proformastatus = PROFORMA::where('PRID',$data['PRID'])->update(['Status'=>'Invoiced']);
		...
		foreach ($details as $detail) {
			...
			if(isset($detail['ITID'])&&$detail['ITID']!=0){
